...
const Loader = ({ fullScreen = false, size = 'md', text = 'Loading...' }) => {
  ...
  const spinner = (
    <div className="flex flex-col items-center justify-center text-center">
      ...
      <div className={`relative ${sizeClasses[size] || sizeClasses.md} flex items-center justify-center`}>
        {/* Outer Orbit Ring 1 */}
        <div className="absolute inset-0 rounded-full border-2 border-zinc-700/50 border-t-zinc-200 animate-[spin_1.5s_linear_infinite]" />
        
        {/* Inner Counter Orbit Ring 2 */}
        <div className="absolute inset-1.5 rounded-full border-2 border-zinc-800 border-b-zinc-400 animate-[spin_1s_linear_infinite_reverse]" />
        
        {/* Center Glowing Core */}
        <div className="w-2.5 h-2.5 bg-zinc-200 rounded-full shadow-[0_0_12px_rgba(255,255,255,0.6)] animate-ping" />
      </div>

      {/* Loading Text */}
      {text && (
        <p className="mt-4 text-xs font-semibold tracking-widest uppercase text-zinc-400 animate-pulse">
          {text}
        </p>
      )}
    </div>
  ...
  if (fullScreen) {
    return (
      <div className="fixed inset-0 min-h-screen w-screen bg-zinc-950/80 backdrop-blur-xl flex items-center justify-center z-[9999] m-0 p-0">
        <div className="px-8 py-6 rounded-3xl bg-zinc-900/90 border border-zinc-800 shadow-[0_0_50px_rgba(0,0,0,0.8)] flex items-center justify-center">
          {spinner}
        </div>
      </div>
    );
