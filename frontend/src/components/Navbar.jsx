...
//     { icon: MessageCircle, href: `https://wa.me/${whatsappNumber}` },
//   ];
...
//       {/* ================= SOCIAL MEDIA BAR (POSITION CHANGED) ================= */}
//       {/* CHANGE LOG: 
//           1. md:top-1/2 ko badal kar md:top-[40%] kiya (Desktop pe upar karne ke liye)
//           2. bottom-6 ko bottom-24 kiya (Mobile pe upar karne ke liye taaki bottom nav se na takraye)
//       */}
//       <div className={`fixed z-[100] transition-all duration-500 
//         md:left-2 md:top-[40%] md:-translate-y-1/2 
//         right-2 bottom-24 md:right-auto md:top-auto md:translate-y-0`}>
...
//           {/* ================= ADMIN LOGIN ICON ================= */}
//           <div className="w-full h-[1px] bg-white/10 my-1 md:my-2"></div>
//           <button
  ...
  return (
    <>
      ...
      <div className={`fixed z-[100] transition-all duration-500 
        ...
        <div className="flex flex-col gap-3 md:gap-5 p-2 bg-[#022c22] border border-emerald-500/20 rounded-full shadow-2xl items-center backdrop-blur-md">
          {socialLinks.map((social, index) => (
            <a
              key={index}
              href={social.href}
              target="_blank"
              rel="noopener noreferrer"
              className="text-emerald-500/60 transition-all duration-300 hover:scale-125 hover:text-emerald-400 active:scale-90 p-1"
            >
              <social.icon size={14} className="md:w-5 md:h-5" strokeWidth={2} />
            </a>
          ))}
          ...
          {/* Divider */}
          <div className="w-full h-[1px] bg-emerald-500/10 my-1 md:my-2"></div>
          <button
            onClick={() => navigate('/admin/login')}
            className="text-emerald-500/60 transition-all duration-300 hover:scale-125 hover:text-emerald-300 active:scale-90 p-1"
            title="Admin Login"
          >
            <Shield size={14} className="md:w-5 md:h-5" strokeWidth={2.5} />
          </button>
      ...
      {/* ================= MOBILE LOGO ================= */}
      <div className="md:hidden fixed top-0 left-0 w-full flex justify-center py-4 z-[100] bg-gradient-to-b from-[#022c22]/80 to-transparent pointer-events-none">
        <Link to="/" className="flex items-center gap-2 px-4 py-1.5 bg-[#022c22] border border-emerald-500/20 rounded-full shadow-lg pointer-events-auto">
          <img src="https://res.cloudinary.com/dsjnikk42/image/upload/v1761223239/Screenshot_20251023-180837.Photos_mwi249.png" className="w-5 h-5 rounded-full" alt="Logo" />
          <span className="font-pacifico text-sm text-white">Kundan</span>
        </Link>
      </div>
      ...
      <nav className="hidden md:flex fixed top-6 w-full justify-center z-40 font-nav-modern">
        ...
        <div className="relative flex items-center bg-[#022c22] border border-emerald-500/20 px-6 py-2 rounded-full shadow-2xl overflow-hidden antialiased-text">
          <Link to="/" className="flex items-center gap-3 pr-6 border-r border-emerald-500/10 group z-20">
            <img
              src="https://res.cloudinary.com/dsjnikk42/image/upload/v1761223239/Screenshot_20251023-180837.Photos_mwi249.png"
              alt="Logo"
              className="w-8 h-8 rounded-full border border-emerald-500/20 group-hover:brightness-125 transition-all"
            />
            <span className="font-pacifico text-2xl text-white">Kundan</span>
          </Link>

          <div className="relative flex items-center ml-2">
            {/* Active Indicator (Emerald) */}
            <div
              className="absolute h-8 rounded-full bg-emerald-600 transition-all duration-500 ease-out shadow-[0_0_15px_rgba(16,185,129,0.4)]"
              style={{ width: "100px", left: `${activeIndex * 100}px` }}
            />
            {navItems.map((item) => (
              <Link
                key={item.path}
                to={item.path}
                className={`relative z-10 w-[100px] text-center py-2 text-[15px] font-medium transition-all duration-300 ${location.pathname === item.path ? "text-white font-bold" : "text-emerald-500/50 hover:text-white"
                  }`}
              >
                {item.label}
              </Link>
      ...
      <div className="md:hidden fixed bottom-6 left-1/2 -translate-x-1/2 z-40 w-[90%] max-w-[320px] font-nav-modern">
        {/* Emerald Green Container */}
        <div className="relative flex items-center bg-[#022c22] border border-emerald-500/20 rounded-full p-1 shadow-2xl overflow-hidden">
          <div
            className="absolute h-[80%] rounded-full bg-emerald-600 transition-all duration-500"
            style={{
              width: `calc(${100 / navItems.length}% - 6px)`,
              left: `calc(${safeIndex * (100 / navItems.length)}% + 3px)`,
            }}
          ...
          {navItems.map((item) => {
            ...
            return (
              <Link
                key={item.path}
                to={item.path}
                className={`relative z-10 flex flex-1 flex-col items-center justify-center py-1.5 transition-all ${isActive ? "text-white" : "text-emerald-500/50"
                  }`}
              >
                <Icon size={16} strokeWidth={isActive ? 2.5 : 2} />
                <span className="text-[9px] font-bold uppercase tracking-tight">{item.label}</span>
