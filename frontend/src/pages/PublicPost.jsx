...
const PublicPost = ({ post: propPost, isHomePage = false }) => {
  ...
  if (loading && !isHomePage) return <div className="min-h-screen bg-[#022c22] flex items-center justify-center text-emerald-500 font-black animate-pulse uppercase tracking-widest">Syncing_Archives...</div>;
  if (!post) return null;
  ...
  return (
    <div className={`relative w-full h-full ${isHomePage ? '' : 'min-h-screen bg-[#022c22] flex items-center justify-center p-4'}`}>
      
      {/* 1. LIGHTBOX */}
      {selectedImgIndex !== null && (
        <div className="fixed inset-0 z-[5000] bg-black flex flex-col animate-in fade-in duration-300">
          <div className="absolute top-0 inset-x-0 z-[5100] p-4 flex items-center justify-between bg-gradient-to-b from-black/90 to-transparent">
            <button onClick={() => setSelectedImgIndex(null)} className="flex items-center gap-2 p-2 px-4 bg-emerald-600 text-white rounded-full font-black text-[10px] uppercase shadow-lg">
              <ArrowLeft size={20} /> Close
            </button>
          </div>
          <div className="flex-1 flex items-center justify-center p-4">
      ...
      {/* 2. REELS CARD VIEW */}
      <div className={`relative w-full overflow-hidden bg-[#011a14] border border-emerald-600/20 transition-all duration-500 shadow-xl
        ${isHomePage ? 'rounded-[1.5rem] aspect-[9/16]' : 'max-w-[380px] w-full aspect-[9/16] rounded-[1.5rem]'}`}>
        ...
        <div className="absolute top-6 right-4 z-[40] flex flex-col gap-3">
          <button 
            onClick={() => setShowInCardDetail(!showInCardDetail)}
            className={`p-2 rounded-full transition-all duration-300 shadow-lg ${showInCardDetail ? 'bg-emerald-600 text-white' : 'bg-white/5 backdrop-blur-md text-white border border-white/10'}`}
          >
            {showInCardDetail ? <X size={18} strokeWidth={3} /> : <Info size={18} strokeWidth={3} />}
          </button>
          <button 
            onClick={handleShare}
            className="p-2 rounded-full bg-white/5 backdrop-blur-md text-white border border-white/10 shadow-lg active:scale-90 transition-all"
          >
            <Share2 size={18} strokeWidth={3} />
          </button>
        </div>
        ...
        {post.teamPhotos?.length > 1 && (
          <div className="absolute top-5 inset-x-0 z-30 flex justify-center gap-1.5 px-8">
            {post.teamPhotos.map((_, i) => (
              <div key={i} className={`h-1 flex-1 rounded-full transition-all duration-300 ${i === currentIndex ? 'bg-emerald-500 shadow-[0_0_8px_#10b981]' : 'bg-white/10'}`} />
            ))}
          </div>
        )}
        ...
        <div 
          ref={scrollRef}
          onScroll={handleScroll}
          className="flex w-full h-full overflow-x-auto snap-x snap-mandatory scrollbar-hide bg-[#011a14]"
        >
          {post.teamPhotos?.map((photo, i) => (
            <div key={i} className="relative min-w-full h-full snap-start flex items-center justify-center bg-[#011a14]">
              <img src={photo.url} alt="post" className="w-full h-full object-contain" />
              {!showInCardDetail && (
                <button 
                  onClick={() => setSelectedImgIndex(i)}
                  className="absolute bottom-40 right-4 p-3 bg-emerald-600/80 backdrop-blur-sm text-white rounded-full shadow-2xl z-30 active:scale-90 transition-transform border border-white/10"
                >
                  <Maximize2 size={16} />
                </button>
              )}
        ...
        {/* 3. FULL DETAIL OVERLAY */}
        <div className={`absolute inset-0 z-[35] bg-[#022c22]/95 backdrop-blur-md transition-all duration-500 ease-in-out p-8 flex flex-col
          ${showInCardDetail ? 'translate-y-0 opacity-100' : 'translate-y-full opacity-0'}`}>
          
          <div className="space-y-5 overflow-y-auto max-h-full py-10 scrollbar-hide">
            <div className="inline-block px-3 py-1 bg-emerald-600 text-white text-[9px] font-black rounded-full uppercase tracking-widest">
              {post.category || 'Featured'}
            </div>
            
            <h2 className="text-2xl font-black text-white italic uppercase tracking-tighter leading-tight border-l-4 border-emerald-600 pl-4">
              {post.title}
            </h2>

            {post.description && (
              <div>
                <p className={`text-emerald-100/60 text-[10px] leading-relaxed ${!isExpanded ? 'line-clamp-3' : ''}`}>
                  {post.description}
                </p>
                {post.description.length > 100 && (
                  <button onClick={() => setIsExpanded(!isExpanded)} className="text-emerald-400 text-[9px] font-black uppercase mt-1 hover:underline">
                    {isExpanded ? "Show Less" : "Read More"}
                  </button>
                )}
              </div>
            ...
            {/* WORKING PROCESS SECTION */}
            {post.workingText && (
              <div className="bg-emerald-950/50 p-4 rounded-xl border border-emerald-500/10">
                <h3 className="text-[9px] font-black text-emerald-600 uppercase tracking-[0.2em] mb-2 flex items-center gap-2">
                  <AlignLeft size={12} className="text-emerald-500" /> Working Process
                </h3>
            ...
            {post.meetingOutput && (
              <div className="bg-emerald-600/10 p-4 rounded-xl border border-emerald-500/20">
                <h3 className="text-[9px] font-black text-emerald-400 uppercase tracking-[0.2em] mb-2 flex items-center gap-2">
                  <ClipboardCheck size={12} /> Meeting Output
                </h3>
                <p className="text-emerald-50 text-[11px] italic font-medium leading-relaxed">
                  "{post.meetingOutput}"
                </p>
              </div>
            )}
        ...
        {/* 4. BOTTOM BAR */}
        {!showInCardDetail && (
          <div className="absolute inset-x-0 bottom-0 z-10 bg-gradient-to-t from-[#011a14] via-[#011a14]/80 to-transparent pt-20 pb-6 px-6 pointer-events-none">
            <div className="pointer-events-auto">
              <div className="flex items-center gap-2 mb-2">
                <div className="w-6 h-6 rounded-full border border-emerald-500/50 overflow-hidden bg-emerald-950">
                  <img src={profileLogo} alt="Profile" className="w-full h-full object-cover" />
                </div>
                <span className="text-emerald-400 text-[9px] font-black uppercase tracking-widest block">
                  @{post.credit} • {displayTime}
                </span>
              </div>
              <h2 className="text-lg font-black text-white italic uppercase tracking-tighter leading-tight line-clamp-1 mb-3">
              ...
              <button onClick={() => setShowInCardDetail(true)} className="flex items-center gap-2 text-[8px] font-black text-white bg-emerald-600 px-3 py-2 rounded-full uppercase tracking-widest active:scale-95 transition-all shadow-lg">
                View All Details <ChevronRight size={12} />
              </button>
            </div>
          </div>
...
const DetailRow = ({ icon, label, value }) => (
  <div className="flex items-center gap-3 text-emerald-100/70">
    <div className="p-1.5 bg-emerald-600/20 rounded-lg text-emerald-500">{icon}</div>
    <div className="flex flex-col">
      <span className="text-[8px] font-black text-emerald-800 uppercase tracking-tighter">{label}</span>
      <span className="text-[10px] font-bold uppercase truncate max-w-[150px]">{value || 'N/A'}</span>
    </div>
  </div>
);
