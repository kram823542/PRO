...
const Home = () => {
  ...
  const fonts = [
    { family: "'Pacifico', cursive", color: "text-emerald-500" },
    { family: "'Dancing Script', cursive", color: "text-slate-100" },
    { family: "'Kaushan Script', cursive", color: "text-emerald-400" },
    { family: "'Lobster', cursive", color: "text-white" },
  ];
  ...
  useEffect(() => {
    ...
    const fontTimer = setInterval(() => setFontIndex((prev) => (prev + 1) % fonts.length), 2500);
    const quoteTimer = setInterval(() => setQuoteIndex((prev) => (prev + 1) % quotes.length), 5000);
    const imgTimer = setInterval(() => setImgIndex((prev) => (prev + 1) % profileImages.length), 2000);
  ...
  return (
    // BG changed to Dark Emerald
    <div className="min-h-screen bg-[#022c22] text-white selection:bg-emerald-500/50 overflow-x-hidden font-sans">

      {/* ZOOM MODAL */}
      {selectedImg && (
        <div className="fixed inset-0 z-[99999] flex items-center justify-center bg-black/95 backdrop-blur-2xl p-4" onClick={() => setSelectedImg(null)}>
          <button onClick={() => setSelectedImg(null)} className="absolute top-6 right-6 p-4 bg-emerald-600 text-white hover:bg-emerald-500 rounded-full transition-all z-[100]">
            <X size={24} strokeWidth={3} />
          </button>
          <img
            src={selectedImg}
            alt="zoom"
            className="max-w-full max-h-[90vh] object-contain rounded-xl shadow-[0_0_50px_rgba(16,185,129,0.2)] animate-in zoom-in-95 duration-300"
            onClick={(e) => e.stopPropagation()}
          />
        </div>
      )}

      {/* Background Glows changed to Green */}
      <div className="fixed inset-0 z-0 pointer-events-none">
        <div className="absolute top-[-10%] right-[-10%] w-[600px] h-[600px] bg-emerald-600/10 blur-[120px] rounded-full" />
      ...
      <style dangerouslySetInnerHTML={{
        __html: `
        @import url('https://fonts.googleapis.com/css2?family=Dancing+Script:wght@700&family=Kaushan+Script&family=Lobster&family=Pacifico&display=swap');
        .glass-header { background: rgba(255, 255, 255, 0.02); backdrop-filter: blur(10px); border-bottom: 1px solid rgba(16, 185, 129, 0.1); }
        .no-scrollbar::-webkit-scrollbar { display: none; }
      `}} />

      {/* HERO SECTION */}
      <section className="relative z-10 pt-16 md:pt-24 pb-12 md:pb-20 px-4 md:px-12 max-w-[1400px] mx-auto overflow-hidden">
        <div className="absolute inset-0 bg-[linear-gradient(to_right,#10b9810a_1px,transparent_1px),linear-gradient(to_bottom,#10b9810a_1px,transparent_1px)] bg-[size:30px_30px] md:bg-[size:40px_40px] [mask-image:radial-gradient(ellipse_60%_50%_at_50%_0%,#000_70%,transparent_100%)] pointer-events-none" />
        ...
        <div className="flex flex-col items-center">
          ...
          <div
            ...
            onClick={() => setSelectedImg(profileImages[imgIndex])}
          >
            <div className="absolute -inset-2 md:-inset-4 border border-emerald-500/20 rounded-[1.5rem] md:rounded-[3rem] group-hover:scale-105 transition-transform duration-700 pointer-events-none" />
            
            <div className="relative w-full h-full rounded-[1rem] md:rounded-[2rem] overflow-hidden bg-[#064e3b] shadow-2xl border border-emerald-500/20">
              {profileImages.map((img, i) => (
                <div
                  key={i}
                  className={`absolute inset-0 transition-all duration-1000 ease-in-out
                ...
                >
                  <img
                    ...
                    className="w-full h-full object-cover grayscale brightness-75 md:brightness-50 group-hover:grayscale-0 group-hover:brightness-100 group-hover:scale-105 transition-all duration-1000"
                  />
                  <div className="absolute inset-0 bg-gradient-to-t from-[#022c22] via-transparent to-transparent opacity-80 md:opacity-60" />
                </div>
              ))}

              <div className="hidden sm:flex absolute top-4 md:top-8 left-4 md:left-8 items-center gap-4 z-20">
                <div className="px-4 py-2 bg-black/40 backdrop-blur-md border border-emerald-500/20 rounded-full">
                  <span className="text-[10px] font-black tracking-[0.3em] text-emerald-400 uppercase">System_Active_v2.6</span>
                </div>
              </div>
              ...
              <div className="absolute bottom-4 left-1/2 -translate-x-1/2 md:translate-x-0 md:left-auto md:bottom-8 md:right-8 flex gap-2 md:gap-3 z-20">
                {profileImages.map((_, i) => (
                  <div
                    key={i}
                    className={`h-1 md:h-1.5 transition-all duration-500 rounded-full ${i === imgIndex ? 'w-8 md:w-12 bg-emerald-500' : 'w-2 md:w-4 bg-white/20'}`}
                  />
                ))}
              </div>
            </div>

            <div className="absolute -bottom-4 -left-2 md:-bottom-6 md:left-12 z-30 bg-emerald-600 p-3 md:p-6 rounded-xl md:rounded-2xl shadow-xl transition-transform active:scale-90 md:group-hover:-translate-y-4 md:group-hover:rotate-6">
              <Terminal size={20} className="text-white md:w-8 md:h-8" />
            </div>
          </div>
          ...
          <div className="w-full max-w-[1200px] z-10 mt-6 md:mt-10">
            <div className="flex flex-col md:flex-row md:items-end justify-between gap-8 md:gap-12">
              <div className="flex-1 text-center md:text-left">
                <div className="inline-flex items-center gap-2 mb-4 md:mb-6 text-emerald-500 font-bold text-[9px] md:text-[10px] tracking-[0.4em] md:tracking-[0.5em] uppercase">
                  <span className="w-6 md:w-8 h-[1px] bg-emerald-500" /> Based in Jharkhand
                </div>
                <h1
                  className={`text-5xl sm:text-7xl md:text-9xl lg:text-[150px] xl:text-[180px] leading-[0.8] mb-6 transition-all duration-1000 tracking-tighter font-black ${fonts[fontIndex].color}`}
                  style={{ fontFamily: fonts[fontIndex].family }}
              ...
              <div className="flex-1 flex flex-col items-center md:items-start text-center md:text-left md:pb-12">
                <div className="relative group max-w-xl mb-10">
                  <div className="absolute -left-4 top-0 h-full w-1 bg-gradient-to-b from-emerald-600 to-transparent hidden md:block rounded-full" />
                  <p className="text-slate-300 text-lg md:text-2xl font-light leading-relaxed md:pl-8 transition-colors group-hover:text-emerald-100 italic">
                    <span className="opacity-80">"Pain builds power.</span>{' '}
                    <span className="text-white font-semibold">Focus creates future."</span>{' '}
                    <span className="block mt-2 text-sm tracking-[0.3em] uppercase font-bold text-emerald-500/80 not-italic">
                      You are the plan
                    </span>
                  </p>
                </div>
                ...
                <div className="flex flex-col sm:flex-row items-center gap-6 w-full">
                  <button
                    onClick={() => navigate('/contactme')}
                    className="group relative overflow-hidden px-8 py-5 bg-emerald-600 rounded-xl transition-all active:scale-95 shadow-[0_10px_30px_-10px_rgba(16,185,129,0.5)] border border-emerald-500/20"
                  >
                    <div className="relative z-10 flex items-center gap-3 text-white">
                      <span className="text-[11px] font-black uppercase tracking-[0.2em]">CONTACT ME </span>
                      <ArrowUpRight size={18} className="transition-transform duration-500 group-hover:translate-x-1 group-hover:-translate-y-1" />
                  ...
                  <div className="flex items-center gap-4 bg-[#064e3b]/40 backdrop-blur-md border border-emerald-500/20 p-2 pr-6 rounded-2xl">
                    <div className="flex -space-x-2 ml-2">
                      {['W', 'E', 'L', 'C', 'O', 'M', 'E'].map((l, i) => (
                        <div key={i} className="w-9 h-9 rounded-full bg-[#064e3b] border border-emerald-500/20 flex items-center justify-center text-[10px] font-bold text-emerald-400 shadow-xl ring-2 ring-[#022c22]">
                          {l}
                        </div>
                      ))}
                    </div>
                    <div className="flex flex-col">
                      <span className="text-[10px] text-emerald-700 font-bold uppercase tracking-tighter">Status</span>
                      <span className="text-[10px] text-emerald-400 font-bold uppercase animate-pulse">Online</span>
                    </div>
                  </div>
                </div>
      ...
      {/* QUOTE SECTION */}
      <section className="relative z-10 py-12 px-6 md:px-12 max-w-[1200px] mx-auto border-t border-emerald-500/10">
        <div className="mb-10">
          <h2 className="text-3xl md:text-5xl font-black uppercase italic tracking-tighter text-white">
            Core <span className="text-emerald-500 font-serif">Principles</span>
          </h2>
        </div>
        <div className="flex flex-col gap-10 md:gap-14">
          {quotes.map((quote, index) => (
            <div key={index} className={`flex w-full ${index % 2 !== 0 ? 'justify-end' : 'justify-start'}`}>
              <div className={`max-w-xl w-full ${index % 2 !== 0 ? 'text-right' : 'text-left'}`}>
                <p 
                  className={`text-slate-300 text-lg md:text-xl font-medium italic py-1 border-emerald-600 ${index % 2 === 0 ? 'border-l-2 pl-5' : 'border-r-2 pr-5'}`} 
                  style={{ fontFamily: "Georgia, serif" }}
                >
                  "{quote.text}"
                </p>
                <div className={`mt-3 flex items-center gap-3 ${index % 2 !== 0 ? 'flex-row-reverse' : 'pl-5'}`}>
                  <div className="h-[1px] w-6 bg-emerald-600/40" />
                  <span className="text-[10px] font-bold tracking-[0.3em] text-emerald-500 uppercase">{quote.author}</span>
                </div>
              </div>
            </div>
          ))}
      ...
      <section className="relative z-10 pt-16 pb-6 px-4 md:px-12 max-w-[1600px] mx-auto">
        <div className="flex flex-col md:flex-row items-center md:items-end justify-between gap-8 mb-12">
          <div className="flex-1 text-center md:text-left">
            <h2 className="text-4xl sm:text-5xl md:text-8xl font-black uppercase italic tracking-tighter text-white">
              WORK <span className="text-emerald-500">Insights</span>
            </h2>
          </div>
          <div className="w-full md:w-auto glass-header p-1.5 rounded-2xl border border-emerald-500/20">
            <div className="flex gap-2 overflow-x-auto no-scrollbar px-2">
              {categories.map(cat => (
                <button key={cat} onClick={() => setSelectedCategory(cat)} className={`px-6 py-2.5 rounded-xl text-[9px] md:text-[10px] font-black uppercase tracking-widest transition-all ${selectedCategory === cat ? 'bg-emerald-600 text-white shadow-lg shadow-emerald-600/30' : 'text-emerald-500/50 hover:text-white hover:bg-emerald-500/10'}`}>
                  {cat}
                </button>
              ))}
            </div>
        ...
        {loading ? (
          <div className="h-[40vh] flex justify-center items-center">
            <div className="w-10 h-10 border-4 border-t-emerald-600 rounded-full animate-spin" />
          </div>
        ) : (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 md:gap-5">
            {filteredPosts.map((post, index) => (
              <div key={post._id} className="group relative bg-[#064e3b]/20 border border-emerald-500/10 rounded-[2rem] p-3 md:p-4 hover:border-emerald-500/40 transition-all duration-500">
                <PublicPost post={post} index={index} isHomePage={true} />
              </div>
            ))}
          </div>
      ...
      {/* FOOTER */}
      <footer className="relative z-10 py-16 px-12 border-t border-emerald-500/10 bg-[#011a14]">
        <div className="max-w-[1600px] mx-auto flex flex-col md:flex-row justify-between items-center gap-12 text-center md:text-left">
          <div>
            <h2 className="text-2xl font-black uppercase italic tracking-widest text-emerald-500">Kundan Kumar</h2>
            <p className="text-emerald-700 text-[10px] mt-2 font-medium uppercase tracking-tighter">"Be educated, be agitated, and be organized."</p>
          </div>
          <p className="text-emerald-900 text-[10px] font-black uppercase tracking-[0.5em]">© 2026 ARCHITECT CORE</p>
        </div>
      </footer>
    </div>
  );
