...
>
  {advicePreview && (
    <div className="space-y-4">
      {/* PREVIEW AREA */}
      <div className="bg-zinc-800/80 p-4 md:p-6 rounded-xl border border-zinc-700/60">
        <AdviceFormate
      ...
      {/* BUTTONS - NICHE (bottom), 2 separate buttons */}
      <div className="flex justify-end gap-3 pt-4 border-t border-zinc-800">
        <button
          onClick={() => setShowPreviewModal(false)}
          className="px-6 py-2.5 bg-zinc-800 hover:bg-zinc-700 text-zinc-200 font-bold rounded-xl border border-zinc-700 transition-colors text-xs uppercase tracking-wider"
        >
          Close
        </button>
        ...
        <button
          onClick={() => {
            if (window.__adviceFormatePrint) {
              window.__adviceFormatePrint();
            } else {
        ...
        <button
          onClick={() => {
            if (window.__adviceFormateDownload) {
              window.__adviceFormateDownload();
            } else {
              toast.error('Download not ready, please wait');
            }
          }}
          className="bg-emerald-600 hover:bg-emerald-500 text-white font-bold py-2.5 px-6 rounded-xl shadow-md transition-all text-xs uppercase tracking-wider flex items-center gap-2 active:scale-95"
        >
          <span>⬇️</span>
